...
function do_KDF_Ready_Individual(event, kdf) {
    ...
    $("label[for='dform_widget_report_type']").attr("tabindex","0");
    
    //Instantiate select2 widget for search inside the service dropdown
    $("#dform_widget_report_type").select2({
        width: '31.5%',
...
function getActivePageName(pageid, itemToReturn) {
    ...
    if (typeof thePage != 'undefined') {
        if (thePageLength > 0) {
            ...
            if (itemToReturn == 'label') {
                ...
                returnValue = thePage[0].innerText;
            }
        }
    }
    return returnValue;
