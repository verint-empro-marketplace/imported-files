/*! Location search widget that supports searching by street and property
    ...
    ----------------------
        ...
        $('#dform_widgets').off('_KDFWidget_locationResultSelected').on('_KDFWidget_locationResultSelected', function(event, kdf, label, value, name) {
        ...
        $('#dform_widgets').off('_KDFWidget_locationSearchResponse').on('_KDFWidget_locationSearchResponse', function(event, kdf, response, action, name, numresults) {
...
(function($) { // Hide scope, no $ conflict
	...
	var definition = {
        ...
        _attachEvents: function(id) {
            ...
            $('#' + id + '_searchbutton').off('click').on('click', function(e) {
                ...
                return $.ajax({
                    ...
                    $( '#dform_'+ KDF.kdf().name ).trigger('_KDFWidget_locationSearchResponse', [ KDF.kdf(), response, widget_instance.options.customaction_search, id, results.length ] );
                    ...
                    $( '#dform_'+ KDF.kdf().name ).trigger('_KDF_customError', [ KDF.kdf().customaction, xhr, settings, thrownError ] );
            ...
            $('#' + id + '_results ').off('change').on('change', function(e) {
                ...
                if(loaddata === true){
                    if(widget_instance.options.search_type == 'property'){
                        KDF.setPropertyID(selectedVal, loaddata, '');
                    } else {
                        KDF.setStreetID(selectedVal, loaddata, '');
                    }
                }
                ...
                $( '#dform_'+ KDF.kdf().name ).trigger('_KDFWidget_locationResultSelected', [ KDF.kdf(), selectedLabel, selectedVal, id ] );
            });
		...
		_buildSearchHolder: function() {
            ...
            h += '<br/><button type="button" id="' + widget_instance.options.id + '_searchbutton" data-type="searchwidget" class="dform_widget  dform_widget_type_button">' + widget_instance.options.search_button_label + '</button>';
            h += '<br/></div>';
            return h;
		},
