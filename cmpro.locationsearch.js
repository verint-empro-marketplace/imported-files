/*! Location search widget that supports searching by street and property
	...
	v3 (17 Jan 2022) - Allow user to choose between a Reset button or a reset icon.
	v4 (10 Feb 2022) - Added an additional event when the search is reset
    ...
    ----------------------
		...
		$('#dform_widgets').off('_KDFWidget_searchReset').on('_KDFWidget_searchReset', function(event, kdf, id) {
            //Fires when search is reset
            console.log("_KDFWidget_searchReset: ID", id);
        });																												
...
(function($) { // Hide scope, no $ conflict
	...
	var definition = {
		options: {
            ...
            search_button_class: '',
			reset_button_label: 'Search again',
            reset_button_class: '',
            reset_button: true, 					  
        ...
        _renderResults: function(id, r) {            
            ...
            $('#' + id + '_results').empty().append('<option value="">'+this.options.first_option_label+'</option>');
		...
		_buildResultsHolder: function() {
            ...
            if(this.options.reset_button ){
                h += '<button type="button" id="' + this.options.id + '_resetbutton" class="dform_widget dform_widget_type_button search_resetresults ' + this.options.reset_button_class + '">';
                h += this.options.reset_button_label + '</button>';
            } else {
                h += '<a href="javascript:void(0);" class="dform_widget_search_closeresults search_resetresults" aria-label="Reset">b</a>'; 
            }
